...
class CursorPlayButton extends InteractiveComponent {
    constructor(element, options = {}) {
        ...
        // ✅ ПРАВИЛЬНО - только базовые свойства в constructor
        // Остальные свойства инициализируются в beforeInit()
        this.isActive = false;
    ...
    beforeInit() {
        ...
        // Привязываем методы к контексту для правильной работы событий
    ...
    bindEvents() {
        ...
        // ✅ ПРАВИЛЬНО - используем InteractiveComponent метод с throttling
        // Это обеспечивает правильную производительность и привязку событий
        this.addThrottledEventHandler(
            document, 
            'mousemove', 
        ...
        this.log('info', 'Mouse events bound with throttling (60 FPS)');
    }
    ...
    setupAnimations() {
        ...
        // ✅ РЕШЕНИЕ: Используем requestAnimationFrame вместо GSAP ticker
        // Это обходит ограничения PerformanceConfig и обеспечивает плавность
        this.animationId = null;
        this.startAnimationLoop();
        
        this.log('info', 'RAF animation loop started (bypassing PerformanceConfig)');
    }
    ...
    handleMouseMove(e) {
        ...
        // DEBUG: показать что события мыши приходят
        if (Math.random() < 0.01) { // 1% логов чтобы не спамить
     ...
     * Обходит ограничения PerformanceConfig для плавного движения
     */
    startAnimationLoop() {
        const animate = () => {
            // Проверяем состояние компонента
            if (this.isDestroyed) {
                this.log('info', 'Animation loop stopped - component destroyed');
                return;
            }
            
            // Обновляем позицию если активен
            if (this.isActive && !this.isMobile && this.button) {
                this.updatePosition();
            }
        ...
        // Запускаем цикл
        this.animationId = requestAnimationFrame(animate);
        this.log('info', 'RAF animation loop started');
    }
    ...
    stopAnimationLoop() {
        if (this.animationId) {
            ...
            this.animationId = null;
            this.log('info', 'RAF animation loop stopped');
        }
    }
    
    /**
     * Обновление позиции кнопки (вызывается через RAF)
     */
    updatePosition() {
        // DEBUG: Увеличиваем частоту логов до 5% для лучшей диагностики
        if (Math.random() < 0.05) { // 5% логов для диагностики RAF
            console.log('🎯 RAF updatePosition:', {
    ...
    activate(onPlayCallback) {
        ...
        try {
            ...
            if (this.button) {
                console.log('🔥 Button exists, ready for activation');
                this.log('info', 'Button ready for activation');
            } else {
    ...
    cleanupAnimations() {
        ...
        // ✅ ИСПРАВЛЕНО: Останавливаем RAF анимационный цикл
        this.stopAnimationLoop();
        
        // Убиваем все GSAP анимации именно нашей кнопки (НЕ this.element!)
        if (typeof gsap !== 'undefined' && this.button) {
